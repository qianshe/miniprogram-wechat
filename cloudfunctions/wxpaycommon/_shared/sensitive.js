...
const createLogger = (functionName) => {
  const formatPrefix = (level, action) => {
    const timestamp = new Date().toISOString()
    return `[${timestamp}] [${functionName}] [${level}]${action ? ` [${action}]` : ''}`
  }
  ...
  return {
     ...
     */
    info: (action, data) => {
      safeLog(formatPrefix('INFO', action), data)
    },
     ...
     */
    warn: (action, data) => {
      safeWarn(formatPrefix('WARN', action), data)
    },
    ...
    error: (action, error, context) => {
      safeError(formatPrefix('ERROR', action), error, context)
    },
    ...
    debug: (action, data) => {
      if (process.env.NODE_ENV === 'development') {
        safeLog(formatPrefix('DEBUG', action), data)
      }
    }
  }
}
